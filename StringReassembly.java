...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //gets rid of overlap ending on string 1
        String short1 = str1.substring(0, str1.length() - overlap);
        String newString = short1 + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //making a copy to iterate through the string set
        Set<String> strSetCopy = strSet.newInstance();
        strSetCopy.transferFrom(strSet);
        boolean setContainsStrWithSubstr = false;
        int containOccurrences = 0;
        while (strSetCopy.size() > 0) {
            String removedStr = strSetCopy.removeAny();
            //removes all strings in the set, if any contain "str" within them,
            //a counter is added
            if (removedStr.contains(str)) {
                containOccurrences++;
            }
            //add the removed string back into the original set
            strSet.add(removedStr);
        }
        //if the counter went up even once, that means the "str" value is
        //contained within the set of strings
        if (containOccurrences > 0) {
            setContainsStrWithSubstr = true;
        }
        //if "str" isn't found in the set, we now must iterate through
        //everything to see if any strings are a substring of "str" itself
        if (!setContainsStrWithSubstr) {
            strSetCopy = strSet.newInstance();
            strSetCopy.transferFrom(strSet);
            while (strSetCopy.size() > 0) {
                containOccurrences = 0;
                String removedStr = strSetCopy.removeAny();
                if (str.contains(removedStr)) {
                    containOccurrences++;
                }
                //only add the string back to the original set if it's not a
                //substring of "str"
                if (containOccurrences == 0) {
                    strSet.add(removedStr);
                }
    ...
    public static Set<String> linesFromInput(SimpleReader input)
        ...
        Set<String> noSubs = new Set1L<>();
        //while the input isn't done being read, take each line as "str"
        //for the method we just completed
        while (!input.atEOS()) {
            String currentLine = input.nextLine();
            addToSetAvoidingSubstrings(noSubs, currentLine);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String correctText = text.replaceAll("~", "\n");
        out.print(correctText);
